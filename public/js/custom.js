$(document).ready(function() {
    ...
    */
    ...
    var prodBatchProfit = [];// Array for batch profit of each product
    var prodBatchLoss = []; // Array for batch loss of each products
        ...
        for (var ch = 0; ch < chosenProduct.length; ch++) {
            if (parseInt(pEntities[index].prodQuantity) === 0) {
                ...
                if (ch === chosenProduct.length - 1) {
                    ...
                    cpCount++;
                    console.log(prodBatchLoss);
                    $('#transaction-body').trigger("transfer:profitLossData");
                }
            } else {
                break;
            }
    ...
    var quantityInput, quantityAvail = [],
        ...
        profitPerItem =[] // New product batch profit per change in product quantity
        ,lossPerItem=[] // New product batch Loss per change in product quantity
        ,totalProfit //Cumulative profit of transaction entiries
        ,totalLoss //Cumulative loss of transaction entries
    $('#transaction-body').on("transfer:profitLossData",function (argument) {
        // body...
        profitPerItem = prodBatchProfit;
        lossPerItem = prodBatchLoss;
    })
    /*
     *@description : Event listener to calculate the total transactions with respect to the product quantity and price
     */
        ...
        $('.quantityInput').each(function(index, element) {
            ...
            $(element).keyup(function(e) {
                ...
                if ((parseInt($(this).val()) <= quantityAvail[index]) || $(this).val().length === 0) { //To check if the input quantity is lower than the available product quantity
                    ...
                    totalProfit = 0;
                    totalLoss = 0;
                    profitPerItem[index] = quantityInput * prodBatchProfit[index];
                    lossPerItem[index] = quantityInput * prodBatchLoss[index];
                    ...
                    $('.batchProfit').eq(index).text(profitPerItem[index]);
                    $('.batchLoss').eq(index).text(lossPerItem[index]);

                    for (var i = 0; i < productCal.length; i++) {
                        totalCal += productCal[i];
                    }
                    for (var i = 0; i < cpCount ; i++) {
                        totalProfit += parseInt(profitPerItem[i]); //cumulative profit
                        totalLoss += parseInt(lossPerItem[i]); //cumulative losss
                    }
                    $('#totalProfit').text(totalProfit); //total profit
                    $('#totalLoss').text(totalLoss);// total Loss
                    $('#transactionTotal').text(totalCal);//total transaction
                } else {
                    $('.response').css({
                        'display': 'block'
    ...
    /*
     *@description: Click listener to delete all transaction entries
     */
